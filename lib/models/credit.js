...
const Credit = {
  ...
  async initCredits(userId) {
    await db.execute({
      sql: "INSERT INTO credits (user_id, balance) VALUES (?, 0) ON CONFLICT(user_id) DO NOTHING",
      args: [userId],
    });
  },
  ...
  async addCredits(userId, amount, description) {
    await db.execute({
      sql: "INSERT INTO credits (user_id, balance) VALUES (?, ?) ON CONFLICT(user_id) DO UPDATE SET balance = balance + ?, updated_at = datetime('now')",
