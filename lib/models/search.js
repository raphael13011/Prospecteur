...
const Search = {
  ...
  async getHistory(userId, limit = 30) {
    const r = await db.execute({ sql: "SELECT id, industry, location, target, lead_count, created_at FROM searches WHERE user_id = ? ORDER BY created_at DESC LIMIT ?", args: [userId, limit] });
    return r.rows;
  },
...
const Lead = {
  async createBulk(searchId, userId, leads) {
    const stmts = leads.map((l) => ({
      sql: "INSERT INTO leads (search_id, user_id, company, website, industry, location, description, size, contact_name, contact_role, email, email_verified, phone, linkedin, score, reason) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      args: [searchId, userId, l.company, l.website || null, l.industry || null, l.location || null, l.description || null, l.size || null, l.contact_name || null, l.contact_role || null, l.email || null, l.email_verified ? 1 : 0, l.phone || null, l.linkedin || null, l.score || 0, l.reason || null],
    }));
  ...
  async getByUser(userId, { page = 1, limit = 50 } = {}) {
    ...
    const r = await db.execute({ sql: "SELECT l.*, s.industry as search_industry, s.location as search_location FROM leads l JOIN searches s ON l.search_id = s.id WHERE l.user_id = ? ORDER BY l.created_at DESC LIMIT ? OFFSET ?", args: [userId, limit, offset] });
