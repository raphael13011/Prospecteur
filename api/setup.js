...
module.exports = async function handler(req, res) {
  ...
  if (key !== process.env.JWT_SECRET?.slice(0, 12)) {
  ...
  try {
    await db.batch([
      `CREATE TABLE IF NOT EXISTS users (
        ...
        password_hash TEXT NOT NULL, name TEXT NOT NULL, company TEXT DEFAULT '',
      ...
      `CREATE TABLE IF NOT EXISTS searches (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
        industry TEXT NOT NULL, location TEXT NOT NULL, target TEXT DEFAULT '',
      ...
      `CREATE TABLE IF NOT EXISTS leads (
        ...
        user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
        company TEXT NOT NULL, website TEXT, industry TEXT, location TEXT, description TEXT,
        size TEXT, contact_name TEXT, contact_role TEXT, email TEXT, phone TEXT, linkedin TEXT,
        score INTEGER DEFAULT 0, reason TEXT,
        created_at TEXT NOT NULL DEFAULT (datetime('now'))
      )`,
    ...
    res.json({ status: "ok", message: "Base de données initialisée.", tables: ["users", "credits", "credit_transactions", "searches", "leads", "password_resets"] });
  } catch (err) {
