...
function Input({ label, error, ...p }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", width: "100%" }}>
      ...
      {error && <span style={{ fontSize: 12, color: "#ef4444", marginTop: 4 }}>{error}</span>}
    </div>
  );
}
...
function AuthModal({ onAuth, onClose, prefill }) {
  ...
  const go = async () => {
    setErr(""); setLd(true);
    try {
      if (mode === "forgot") { await api("/auth?action=forgot-password", { method: "POST", body: { email } }); setMode("sent"); setLd(false); return; }
...
function BuyCredits({ token, onDone }) {
  ...
  const buy = async (packId) => {
    setBuying(packId);
    try { const d = await api("/billing?action=checkout", { method: "POST", body: { pack: packId }, token }); if (d.checkout_url) window.open(d.checkout_url, "_blank"); } catch (e) { alert(e.message); }
    setBuying(null);
  };
  return (
    <div>
      ...
      {onDone && <button className="gb" style={{ ...S.gBtn, marginTop: 12 }} onClick={onDone}>← Retour</button>}
    </div>
  );
}
...
export default function App() {
  ...
  const search = async () => {
    ...
    try {
      const d = await api("/leads?action=generate", { method: "POST", token, body: { industry, location, target, count: parseInt(count) || 10 } });
      setResults({ leads: d.leads, meta: d.meta }); setBalance(d.balance);
      try { const h = await api("/leads?action=history", { token }); setHistory(h.searches || []); } catch {}
        ...
        <div>
          ...
          {history.map(h => (
            <div key={h.id} className="hi" style={S.histI} onClick={() => { setIndustry(h.industry); setLocation(h.location); setTarget(h.target || ""); setView("search"); }}>
              <div>
                <div style={{ fontWeight: 600, fontSize: 14 }}>{h.industry}</div>
                <div style={{ fontSize: 13, color: "#64748b" }}>{h.location} · {h.lead_count} leads</div>
              </div>
              ...
              {results.leads.map((l, i) => (
                <div key={i} className="lc" style={{ ...S.lCard, animationDelay: `${i * .04}s`, ...(expandedId === i ? { borderColor: "#818cf8" } : {}) }} onClick={() => setExpandedId(expandedId === i ? null : i)}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 15, fontWeight: 700, color: "#0f172a" }}>{l.company}</div>
                      <div style={{ fontSize: 12, color: "#94a3b8" }}>{l.industry} · {l.location}</div>
                    </div>
                  ...
                  {expandedId === i && (
                    <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px solid #f1f5f9", animation: "slideUp .2s ease" }}>
